...
const isMajority = computed(() => {
  ...
  const playersInGame = llistaJugadors.value.filter((p) => p.role === 'player' || p.role === 'admin')
...
function startGame() {
    ...
    if (socket.value && socket.value.connected && isAdmin.value && isMajority.value) {
        ...
        console.error("[viewLobby] No se pudo iniciar partida: Socket desconectado, no es Admin, o falta mayoría.");
    }
}
